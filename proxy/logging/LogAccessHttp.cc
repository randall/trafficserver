...
LogAccessHttp::LogAccessHttp(HttpSM *sm)
  : m_http_sm(sm),
    ...
    m_proxy_resp_result_reason_str(nullptr),
    m_proxy_resp_result_reason_len(0),
...
LogAccessHttp::init()
{
  ...
  if (hdr->client_response.valid()) {
    ...
    m_proxy_resp_result_reason_str = (char *)m_proxy_response->reason_get(&m_proxy_resp_result_reason_len);
  }
...
int
LogAccessHttp::marshal_proxy_resp_result_reason(char *buf)
{
  int len = round_strlen(m_proxy_resp_result_reason_len + 1);
  if (buf) {
    marshal_mem(buf, m_proxy_resp_result_reason_str, m_proxy_resp_result_reason_len, len);
  }
  return len;
}
