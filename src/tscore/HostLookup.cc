  ...
  while (true) {
    ...
    if (state.index >= numLegalChars) {
      if (cur_level <= 0) {
        ...
        //state = q[--cur_level];
        state = q[cur_level - 1] ; // mabye this?
        cur_level--;
      ...
      //
      if (state.block->array[state.index].branch != nullptr && first_element == false) {
          break;
...
/*
    else if (state.block->array[state.index].branch != nullptr) {
...
HostLookup::InsertBranch(HostBranch *insert_in, string level_data)
{
  ...
  case HostBranch::HOST_ARRAY: {
    ...
    if (insert_in->next_level._array->Insert(level_data, new_branch) == false) {
      // The array is out of space, time to move to a hash table
      HostArray *ha = insert_in->next_level._array;
