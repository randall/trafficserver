...
AC_DEFUN([TS_CHECK_CRYPTO], [
  ...
  if test "x${enable_crypto}" = "xyes"; then
    temp_OPENSSL_LIBS=$OPENSSL_LDFLAGS
    TS_ADDTO(temp_OPENSSL_LIBS, [$OPENSSL_LIBS])
    OPENSSL_LIBS=$temp_OPENSSL_LIBS
    TS_ADDTO(OPENSSL_INCLUDES, [$OPENSSL_INCLUDES])
    TS_ADDTO(OPENSSL_INCLUDES, [-DOPENSSL_NO_SSL_INTERN])
  fi
