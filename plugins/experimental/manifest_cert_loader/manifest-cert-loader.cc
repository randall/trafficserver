...
extern SSL_CTX *SSLDefaultServerContext();
extern bool SSL_CTX_add_extra_chain_cert_bio(SSL_CTX *ctx, BIO *bio);
...
Load_Certificate(SSLEntry const *entry, std::deque<std::string> &names)
{
  ...
  if (config == nullptr) {
    TSDebug(PN, "Failed to load cert file %s; could not get config", entry->certFileName.c_str());
    return nullptr;
  }
